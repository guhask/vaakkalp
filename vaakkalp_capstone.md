...
# Tools exposed:
#   - detect_language(audio_text) -> {language_code, confidence, dialect_hint}
...
VaakKalp is containerized with Docker and deployed on **Google Cloud Run** in the
`asia-south1` region, with **Vertex AI** as the model backend for all agent reasoning.
...
| Deployability | Video | Cloud Run deployment demo with logging |
...
connect to the UNESCO Language Atlas, Google Translate and Bhashini for low-resource
...
*VaakKalp is deployed on Google Cloud Run, backed by Vertex AI, with full Cloud
Logging observability. One script deploys the entire system.*

[Show deploy.sh running, Cloud Run URL appearing]"*
...
| 6 | Jun 26 | Translation MCP Server (Google Translate + Bhashini) | MCP connected and tested | ☐ |
...
| 13 | Jul 3 | Cloud Run deployment + Dockerfile + deploy.sh | Live public URL on Cloud Run | ☐ |
| 14 | Jul 4 | Agent Skills (Cultural Interviewer + Archivist) | Skills exportable via CLI | ☐ |
| 15 | Jul 5 AM | Video recording (≤5 min) → Upload to YouTube | YouTube link live | ☐ |
| 16 | Jul 5 PM | Kaggle Writeup + README.md polish | Writeup saved as draft | ☐ |
| 17 | Jul 6 | Final review → SUBMIT before 11:59 PM PT | ✅ SUBMITTED | ☐ |
...
6. **Google ecosystem alignment** — Google Translate, Bhashini (which Google supports),
   Vertex AI, Cloud Run, Cloud Storage — this is a fully Google-native stack. Judges from
   Google will recognize and appreciate this.
